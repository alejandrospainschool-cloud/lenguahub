  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 md:px-8 space-y-10 animate-in fade-in duration-500 pb-12">
      ...
      >
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-indigo-500/20 rounded-lg text-indigo-300"><Languages size={22} /></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1fr,auto,1fr] gap-4 items-center">
             ...
             <div className="hidden md:flex justify-center text-amber-400">{isTranslating ? <Loader2 className="animate-spin" /> : <ArrowRight />}</div>

             <div className="space-y-2 relative">
               <label className="text-xs font-bold text-rose-300 uppercase ml-2">Spanish</label>
               <div className="w-full h-[62px] bg-[#0f172a]/80 border border-amber-500/10 rounded-2xl px-4 text-lg flex items-center text-white">{translatedText || <span className="text-slate-600">Translation...</span>}</div>
               {translatedText && !translatedText.startsWith('Error') && !hasSaved && (
                 <button onClick={() => setShowSaveModal(true)} className="absolute right-2 top-8 p-2 bg-rose-600 hover:bg-rose-500 text-white rounded-xl shadow-lg active:scale-95 transition-all"><Save size={18} /></button>
               )}
             </div>
          </div>
          
          <div className="flex justify-end mt-4">
             <button onClick={handleTranslate} disabled={isTranslating || !inputText.trim()} className="px-6 py-3 bg-indigo-600 rounded-xl text-white font-bold hover:bg-indigo-500 transition disabled:opacity-50">
        ...
        {/* SAVE MODAL */}
        {showSaveModal && (
          <div className="absolute inset-0 z-50 rounded-3xl bg-[#02040a]/95 backdrop-blur-sm flex items-center justify-center p-4">
              ...
              {!isNewFolder ? (
                 <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto">
                   {existingFolders.map(f => (
                     <button key={f.name} onClick={() => setTargetFolder(f.name)} className={`p-3 rounded-xl border text-left text-sm font-medium ${targetFolder === f.name ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-slate-800 border-slate-700 text-slate-300'}`}>
                       <span className="w-2 h-2 rounded-full inline-block mr-2" style={{backgroundColor: f.color}}/>{f.name}
                     </button>
                   ))}
                   <button onClick={() => {setIsNewFolder(true); setTargetFolder('')}} className="p-3 rounded-xl border border-dashed border-slate-600 text-slate-400 flex items-center justify-center gap-2"><FolderPlus size={16}/> New</button>
                 </div>
              ) : (
                 <div className="flex gap-2">
                   <input autoFocus value={targetFolder} onChange={e => setTargetFolder(e.target.value)} placeholder="Folder Name" className="flex-1 bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white" />
                   <button onClick={() => setIsNewFolder(false)} className="text-slate-400">Cancel</button>
                 </div>
              )}
      ...
      <header className="text-center space-y-6">
        ...
        <div className="max-w-xl mx-auto bg-[#1e293b] p-4 rounded-2xl border border-slate-800">
           ...
           <div className="h-3 bg-slate-700/50 rounded-full overflow-hidden">
             <div className="h-full bg-gradient-to-r from-amber-500 to-orange-400 rounded-full transition-all duration-1000" style={{ width: `${progressPct}%` }} />
           </div>
        </div>
      </header>
    ...
    <div onClick={onClick} className="group relative rounded-3xl p-1 cursor-pointer transition-transform duration-300 hover:-translate-y-1">
      ...
      <div className="relative z-10 p-6 flex flex-col h-full justify-between">
        <div className="flex items-center gap-4 mb-6">
          <div className="p-3 bg-white/5 rounded-2xl text-cyan-300 shadow-inner group-hover:bg-white/10">{React.cloneElement(icon, { size: 24 })}</div>
        ...
        <button className="w-full py-3 bg-white text-slate-900 font-bold rounded-xl shadow-lg hover:bg-slate-100 transition-all flex items-center justify-center gap-2">{btnText} <ArrowRight size={16} className="opacity-40" /></button>
      </div>
    </div>
  )
}