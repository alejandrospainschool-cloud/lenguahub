...
export default function Tools({
  ...
  const [activeTab, setActiveTab] = useState('vocab') // vocab | breakdown | summary
  ...
  const [pendingItem, setPendingItem] = useState(null) // if null => Save all flow

  // Existing user categories (folders)
  ...
  const vocabItems = useMemo(() => {
    if (results?.type !== 'vocab') return []
    return clampItems(results.items || [], 10)
  }, [results])
  ...
  const canGenerate = useMemo(() => {
    ...
    const blocked = hasReachedLimit(dailyUsage, 'aiRequests', isPremium)
    return hasText && !isProcessing && !blocked
  }, [inputText, isProcessing, dailyUsage, isPremium])

  const placeholder =
    activeTab === 'vocab'
      ? "Paste Spanish text here… I'll extract useful vocabulary."
      : activeTab === 'breakdown'
      ? "Paste a Spanish sentence here… I'll break down every word and phrase."
      : "Paste a longer text here… I'll summarize it."
...
${inputText}`
        ...
        if (!items.length) {
          setResults({ type: 'summary', content: 'Could not parse vocab JSON.\n\n' + out })
        } else {
  ...
  const applyFolderChoice = async (category) => {
    if (!category) return
    const chosen = String(category).trim()
    if (!chosen) return

    if (pendingItem) {
      await handleSaveToBank(pendingItem, chosen)
    } else {
      // Save-all case
      const toSave = vocabItems.filter((i) => !savedIDs.includes(i.id))
      for (const item of toSave) {
        // sequential is safer for rate limits + nicer toast behaviour
        // (you can parallelize later if you want)
        // eslint-disable-next-line no-await-in-loop
        await handleSaveToBank(item, chosen)
      }
    }
  ...
  const createNewFolderAndSave = async () => {
    ...
    if (!name?.trim()) return
    await applyFolderChoice(name.trim())
  }
      ...
      <div className="pt-4">
        ...
        <div className="mt-6 flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="bg-[#0f172a] p-1.5 rounded-2xl border border-white/10 flex relative w-full md:w-auto overflow-x-auto">
            ...
            <div
              className="absolute top-1.5 bottom-1.5 bg-gradient-to-r from-purple-600 to-indigo-600 rounded-xl transition-all duration-300 pointer-events-none"
              style={{
          ...
          {!results && !isProcessing && (
            <div className="min-h-[420px] flex flex-col items-center justify-center border-2 border-dashed border-white/5 rounded-3xl bg-white/[0.02] text-center p-8">
              ...
              <p className="text-slate-500 max-w-sm">
                Paste text on the left and hit{' '}
                <span className="text-slate-300 font-bold">Generate</span>.
              </p>
            </div>
          )}
            ...
            <div className="bg-[#0f172a] border border-white/10 rounded-3xl p-7 md:p-8 animate-in slide-in-from-bottom-4 relative overflow-hidden">
              ...
              <div className="flex items-start justify-between gap-4 mb-6">
                ...
                <button
                  ...
                  className="text-slate-300 hover:text-white text-sm font-bold flex items-center gap-2"
                >
                  <Copy size={16} /> Copy
                </button>
              </div>
            ...
            <div className="space-y-6 animate-in slide-in-from-bottom-4">
              <div className="bg-[#0f172a] border border-white/10 rounded-3xl p-8 relative overflow-hidden">
                ...
                <div className="relative z-10">
                  <div className="text-xs uppercase tracking-wider text-indigo-400 font-bold mb-2">
                    Spanish Sentence
                  </div>
              ...
              {Array.isArray(results.data?.parts) && results.data.parts.length > 0 && (
                <div className="bg-[#0f172a] border border-white/10 rounded-3xl p-8">
                  <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
                    <Brain size={20} /> Word-by-Word Breakdown
                  </h3>

                  <div className="space-y-3">
                    {results.data.parts.map((part, idx) => {
                      // Normalize part into a vocab-like item for saving
                      const saveItem = {
                        id: `breakdown:${(part.word || '').toLowerCase()}:${idx}`,
                        term: String(part.word || '').trim(),
                        translation: String(part.translation || '').trim(),
                        definition: String(part.notes || '').trim(),
                      }

                      return (
                        <div
                          key={idx}
                          className="bg-[#1e293b]/50 border border-slate-700/50 rounded-2xl p-4 hover:border-indigo-500/40 transition-colors"
                        >
                          ...
                          <div className="mt-3 flex flex-wrap gap-2">
                            <button
                              onClick={() => handleSaveClick(saveItem)}
              ...
              {Array.isArray(results.data?.phrases) && results.data.phrases.length > 0 && (
                <div className="bg-[#0f172a] border border-white/10 rounded-3xl p-8">
                  <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
                    <BookOpen size={20} /> Phrase Explanations
                  </h3>
                  <div className="space-y-3">
                    {results.data.phrases.map((phrase, idx) => (
                      <div
                        key={idx}
                        className="bg-[#1e293b]/50 border border-slate-700/50 rounded-2xl p-4"
                      >
              ...
              <div className="flex items-center justify-between gap-3 flex-wrap">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-4 py-2 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 text-slate-200 font-bold flex items-center gap-2"
                  >
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:opacity-90 text-white font-bold flex items-center gap-2 disabled:opacity-40"
